...
col2.metric("Active users", f"{movies_statistics.get("users_ratings").get("now").loc["nunique"].values[0]: ,}",
                            f"{movies_statistics.get("users_ratings").get("delta_previous_month").loc["nunique"].values[0]: ,} vs prev. month"
            )

col3.metric("User rating count", f"{movies_statistics.get("users_ratings").get("now").loc["count"].values[0]: ,}",
...
with st.container():
    ...
    with col3:
        ...
        st.bar_chart(preserve_sorting_df(movies_statistics.get("watching_counts"), cat_column='genre'), x='genre', y='count', height = 320, y_label="Movies count", x_label="Genre", color='#BC80BD')

    with col4:
        st.markdown("### Movies count by genre")
        st.plotly_chart(pie_chart(movies_statistics.get("genres_count"), category='genre', value='count'),
...
with st.container():
    ...
    with col5:
        ...
        st.bar_chart(movies_statistics.get("movies_ratings_distribution"), x='rating', y='count', height = 280, y_label="Users count",  x_label="Rating", color="#FDB462")
    with col6:
        st.markdown("### Users activity distribution")
        st.plotly_chart(user_activity_hist(movies_statistics.get("user_activity")), use_container_width=True)
